...
#
# 流程:
#   1. 确保 vLLM 服务已启动 (bash scripts/start_vllm_server.sh)
#   2. Pipeline 推理: LLM + Mask Transformer → dense motion tokens
#   3. Token 重建: RVQVAE 解码 → BVH/JSON
#
# 用法:
...
MASK_CKPT="${MASK_CKPT:-${PROJECT_DIR}/checkpoints/mask_transformer}"

# RVQVAE checkpoint
RVQVAE_CKPT="${RVQVAE_CKPT:-${PROJECT_DIR}/checkpoints/rvqvae/model/epoch_30.pth}"
...
echo "  vLLM URL:       ${VLLM_URL:-http://localhost:${VLLM_PORT}}"
...
CUDA_VISIBLE_DEVICES=${GPU_ID} python pipeline_infer.py \
    ...
    --keyframe_len_policy "${KEYFRAME_LEN_POLICY:-strict}" \
...
CUDA_VISIBLE_DEVICES=${GPU_ID} python reconstruct_from_tokens.py \
    ...
    ${FACE_RVQVAE_CKPT:+--face_rvqvae_ckpt "${FACE_RVQVAE_CKPT}"} \
    ${FACE_INFILL_CKPT:+--face_infill_ckpt "${FACE_INFILL_CKPT}"} \
    --audio_feat_dir "${DATA_DIR}/audio_features_hubert_layer9_fps10"
