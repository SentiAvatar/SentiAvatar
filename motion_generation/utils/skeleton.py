...
class Skeleton(object):
    ...
    def inverse_kinematics_np(self, joints, face_joint_idx, smooth_forward=False):
        ...
        root_quat = qbetween_np(forward, target) # (frames, 4)

        '''Inverse Kinematics'''
        # quat_params (frames, joints_num, 4)
        # print(joints.shape[:-1])
        quat_params = np.zeros(joints.shape[:-1] + (4,))
        # print(quat_params.shape)
        root_quat[0] = np.array([[1.0, 0.0, 0.0, 0.0]]) # 把第0帧的四元数置为1.0？？为啥这样啊
        quat_params[:, 0] = root_quat # 所有帧的
